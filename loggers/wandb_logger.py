...
class WandbLogger(Logger):
    ...
    def _get_public_run(self):
        if self._public_run is None:
            ...
            runpath = experiment.entity + '/' + experiment.project + '/' + experiment.id
            api = wandb.Api()
    ...
    def _scan_and_log_checkpoints(self, checkpoint_callback: "ReferenceType[ModelCheckpoint]", save_last: bool) -> None:
        ...
        for time_, path, score in checkpoints:
            ...
            # remember logged model - timestamp needed in case filename didn't change (last.ckpt or custom name)
    ...
    def _rm_but_top_k(self, top_k: int):
        # top_k == -1: save all model
        # top_k == 0: no model saved at all. The checkpoint callback does not return checkpoints.
        # top_k > 0: keep only top k model (last and best will not be deleted)
